...
Spring Boot (Java) で実装したバックエンド REST API 群を、AWS ALB + ECS Fargate 上で動かすサンプル。
**アプリケーション分離**（Spring Boot アプリ・Docker Image・ECS Service をアプリごとに分離しつつ、
ALB と NLB は共有する構成）のサンプルとして、2本のアプリを収容するマルチアプリ構成になっている。

| アプリ | パス | DB | APM |
| --- | --- | --- | --- |
| `user-company-api` | `apps/user-company-api/` | DynamoDB（ユーザー）、MySQL/Aurora MySQL（会社） | ADOT Java Agent + CloudWatch Agent サイドカーで Application Signals / X-Ray と連携 |
| `log-api` | `apps/log-api/` | なし | なし（アプリケーション分離の最低限のサンプルのため未計装） |
...
- ECS Cluster・ALB・NLB は全アプリで**共有**する（`cloudformation/06-shared-cluster-lb.yaml`）。
  ECS Service・TaskDefinition・Docker Image・ECR リポジトリはアプリごとに分離している。
- ALB → NLB は、NLB の各 AZ ノードに割り当てた固定プライベート IP（Application Subnet の CIDR 内）を
  ...
  仕組みがないための代替手段）。NLB は L4 でパスベースルーティングができないため、
  **アプリごとに NLB のリスナーポートを分けて**（`user-company-api`=80、`log-api`=8081）
  トラフィックを分離している。ALB 側は path-pattern の ListenerRule でアプリごとの
  ポート宛ターゲットグループに振り分ける。
- DB（NoSQL）: DynamoDB（Partition Key = `email`）。VPC からは Gateway 型エンドポイント経由。
- DB（リレーショナル）: ローカル開発は MySQL 8.0 in Docker。AWS 本番は Aurora MySQL。
- 認証: なし（サンプル用途）。
...
```
apps/
  user-company-api/   # ユーザー管理 API（DynamoDB）+ 会社情報管理 API（MySQL）
    ...
    Dockerfile           # Amazon Corretto 17 マルチステージビルド → 非 root で起動（ADOT Java Agent 同梱）
    docker-compose.yml   # ローカル開発用コンテナ定義（MySQL 8.0 + DynamoDB Local）
    mysql/init/          # MySQL 初期化 SQL（companies テーブル DDL）
  log-api/             # 任意のログを受け付けて標準出力するだけの最小サンプル API
    src/main/java/com/example/logapi/
      controller/  # /logs（ログ受信）, /health
      config/      # AppProperties
    Dockerfile           # Amazon Corretto 17 マルチステージビルド → 非 root で起動（APM 計装なし）

cloudformation/    # スタック分割した CFN テンプレート（01〜03,05: 共有インフラ、06: 共有ECS基盤、07/08: アプリ別ECS Service）
scripts/           # ビルド・デプロイ用シェルスクリプト / ローカル開発スクリプト（多くが <app> 引数を取る）
```
...
アプリごとに `apps/<app>/` ディレクトリで独立してビルド・起動できる。
`scripts/local-run.sh` は第1引数にアプリ名（`user-company-api` または `log-api`）を取る。

```bash
# user-company-api（MySQL + DynamoDB Local を自動起動してから Spring Boot 起動）
bash scripts/local-run.sh user-company-api

# log-api（外部依存なし。Spring Boot をそのまま起動）
bash scripts/local-run.sh log-api
```

起動後、`http://localhost:8080/health` で疎通確認できる（両アプリともポート 8080 なので、
同時に2本を起動する場合はどちらかの `server.port` を変更すること）。

**ローカル環境の構成（user-company-api）:**
- MySQL 8.0（Docker, port 3306）— Company API が接続。初回起動時に `apps/user-company-api/mysql/init/01_init.sql` で DDL を自動実行。
- DynamoDB Local（Docker, port 8000）— User API が接続。`users` テーブルは `local-infra.sh` が自動作成。

これらのコンテナだけを個別に起動・停止したい場合:

```bash
bash scripts/local-infra.sh          # 起動 + DynamoDB テーブル作成
bash scripts/local-infra.sh --stop   # 停止
```

**IDE から直接起動する場合の環境変数（user-company-api）:**
...
**IDE から直接起動する場合の環境変数（log-api）:** `APP_ENV=local` のみ（外部依存なし）。

## テスト

テストはアプリごとに独立している（`apps/<app>/` が個別の Maven プロジェクト）。
...
`@WebMvcTest` によるコントローラーテストのみ（外部依存がないため Docker 不要）。
...
### Log API（アプリケーション分離サンプル・APM 計装なし）

任意の内容（Content-Type 不問）を受け付けて標準出力するだけの最小サンプル。

| Method | Path | 概要 |
| --- | --- | --- |
| GET | `/health` | ALB/NLB ヘルスチェック |
| POST | `/logs` | 任意のログを受け付けて標準出力（202 Accepted） |

## デプロイ

AWS 認証情報（環境変数で渡す）と Docker / AWS CLI が必要。
ECS Service・Docker Image・デプロイはアプリ（`user-company-api` / `log-api`）ごとに分離している。
ECS Cluster・ALB・NLB は共有インフラとして `deploy-infra.sh` の中でまとめてデプロイされる。
...
# 1) VPC / ECR（全アプリ分）/ DynamoDB / RDS / 共有 ECS Cluster+ALB+NLB を作成
./scripts/deploy-infra.sh

# 2) Docker イメージをビルドして ECR にプッシュ（アプリ単位）
./scripts/build.sh user-company-api
./scripts/build.sh log-api

# 3) ECS サービスをデプロイ（既存なら新タスクへロールアウト、アプリ単位）
./scripts/deploy-service.sh user-company-api
./scripts/deploy-service.sh log-api

# まとめて（上記を全アプリ分ループ実行）：
./scripts/deploy-all.sh
```

アプリを1本だけ更新したい場合は、そのアプリの `build.sh` / `deploy-service.sh` だけを実行すればよい
（他アプリの ECS Service には影響しない）。

完了後、`scripts/deploy-service.sh <app>` の出力にある ALB DNS 名で疎通確認できる
（`http://<alb-dns>/health`、`http://<alb-dns>/logs` など、アプリごとに ALB のパスで振り分けられる）。

## トレーシング（AWS Application Signals）

> このセクションは `user-company-api` にのみ適用される。`log-api` はアプリケーション分離の
> 最低限のサンプルのため APM 計装（ADOT Java Agent / CloudWatch Agent サイドカー）を組み込んでいない。
...
### Fargate サイドカー構成（`cloudformation/07-ecs-user-company-api.yaml`）
...
### ADOT Java Agent（`apps/user-company-api/Dockerfile`）
...
スクリプトが確認プロンプトを表示した後、各アプリの ECR イメージを削除してからスタックを逆順
（ECS[全アプリ] → 共有基盤(ECS Cluster/ALB/NLB) → RDS → DynamoDB → ECR[全アプリ] → Network）で削除する。
